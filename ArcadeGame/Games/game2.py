...
class GameView(arcade.View):
    ...
    def on_draw(self):
        ...
        self.text_box(
            4.5, 1, 1, arcade.color.PINK, str(self.source)
        )  # print source node
        self.text_box(
            4.5, 11, 1, arcade.color.PINK, str(self.target)
        )  # print target node
        ...
        self.text_box(
            5, 15, 6, arcade.color.PINK, "High Score: {}".format(self.high_score)
        )

        for i, edge in enumerate(self.link_grid):  # print links next to link grid
            self.text_box(
                12, i + 3, 2, arcade.color.PINK, "{}-{}".format(edge[0], edge[1])
            )

        for i, edge in enumerate(self.ans_grid):  # print links next to answer grid
            self.text_box(
                9, i + 3, 2, arcade.color.PINK, "{}-{}".format(edge[0], edge[1])
            )
    ...
    def text_box(self, col, row, width, colour, text=None):
        ...
        if width % 2 == 0:
            arcade.draw_rectangle_filled(
                (col + width / 2) * (MARGIN + WIDTH) - (MARGIN + WIDTH) / 2,
                row * (MARGIN + HEIGHT),
        ...
        if text:  # print text
            arcade.draw_text(
                text,
                col * (MARGIN + WIDTH) - (WIDTH / 4),
                row * (MARGIN + HEIGHT) - (WIDTH / 4),
                arcade.color.BLACK,
                12,
            )
    ...
    def on_mouse_press(self, x, y, button, key_modifiers):
        ...
        """
        self.column = int(
            (x + (WIDTH + MARGIN) / 2) // (WIDTH + MARGIN)
        )  # sets the column the mouse was pressed in
        self.row = int(
            (y + (HEIGHT + MARGIN) / 2) // (HEIGHT + MARGIN)
        )  # sets the row the mouse was pressed in

        if (
            self.column > 11 and self.column < 22 and self.row > 2 and self.row < 11
        ):  # If one of the links in the link grid were clicked
            ...
            self.row == 2
            and self.column < COLUMN_COUNT - self.slots + 2
            and self.column > 0
        ):  # spectrum grid clicked
    ...
    def is_solution(self, ans_grid, first_slot):
        ...
        if (
            path[0] != self.source
            or path[-1] != self.target
            or nx.is_path(self.G, path) == False
        ):
            return False
